...
if __name__ == '__main__':
    ...
    while not os.path.isfile('endgame.txt'): #main python file to run our code from
        ...
        time.sleep(.1)
        if os.path.isfile('GROUPNAME.go'): #main python file to run our code from
            ...
            #Otherwise, a move has been made. Process the latest move and update our board
                ...
                board.placeStone(opponent, optimalMove[0],optimalMove[1])
                #lastly write to the file
                file = open("move_file.txt", "w")
                file.write('GROUPNAME'+str(optimalMove[0])+str(optimalMove[1]))
                file.close()
